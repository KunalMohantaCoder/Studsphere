...
export default function ExplorePage() {
  ...
  useEffect(() => {
    api.get('/users/suggestions').then(r => setUsers(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  const handleFollow = async (userId) => {
    try {
      ...
      setUsers(prev => prev.map(u => u._id === userId ? { ...u, followers: r.data.following ? [...(u.followers || []), 'me'] : (u.followers || []).slice(0, -1) } : u));
    } catch {}
  };
          ...
          {users.map(u => (
            <div key={u._id} className="card" style={{ padding: 20, display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4 }}>{u.followers?.length || 0} followers</div>
                {u.skills?.length > 0 && (
